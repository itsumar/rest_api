...
abstract class REST_Controller extends CI_Controller
{
    ...
    public function __construct($config = 'rest')
    {
        ...
        // Get the language => FROM REST CONFIG FILE
        // $language =  $this->config->item('rest_language');

        // Getting app default language
        $lang_id = $this->post('language_id');
        $lang_id = (!empty($lang_id) && $lang_id == '1') ? '2' : '1';
        $app_lang = $this->rest->db
            ->where('id', $lang_id)
            ->get($this->config->item('rest_lang_table'))->row_array();
        $language = strtolower($app_lang['name']);
        if (!empty($language) && $language != 'arabic') {
            $language = 'english';
        }
    ...
    protected function _detect_api_key()
    {
        ...
        if (($key = isset($this->_args[$api_key_variable]) ? $this->_args[$api_key_variable] : $this->input->server($key_name))) {
            ...
            isset($row->key_activated) && $this->rest->key_activated = $row->key_activated;
            ...
            //Check if key is activated or deactivated
            if (isset($row->key_activated)) {

                if ($row->key_activated == FALSE) {
                    return FALSE;
                }
    ...
    /**
     * Sending email to given email address
     * @param $user_data
     */
    public function send_email_to_user($email_to, $title, $subject, $message)
    {
        ...
        $this->email->from('info@malaeb-ole.ae', $title);
    ...
    function convert2english($string)
    {
        ...
        $result = str_replace($persian, $newNumbers, $string);
        return $result;
    }
    ...
    /**
     * OLE CURL request.
     * @param $url |$fields|$headers
     * @return bool|string|array
     */
    public function curl_request($url, $fields, $headers)
    {
        //CULR REQUEST TO Firebase Cloud Messaging
        $ch = curl_init();
     ...
     * @return bool|string
     */
    public function create_uuid($length = 6)
    {
        $uuid = substr(number_format(time() * mt_rand(), 0, '', ''), 0, $length);
        return $uuid;
    }

    /**
     * unscaping string or removing slaches
     * json string with slashes
     * @param $value
     * @return array
     */
    public function unscape_json_string($value)
    {
        $result = trim($value, '"');
     ...
     */
    public function success_reponse($msg_for, $response, $lang = true)
    {
        $msg = ($lang == false) ? $msg_for : $this->lang->line($msg_for);
     ...
     * Empty result
     */
    public function error_response($msg_for, $response, $lang = true)
    {
        $msg = ($lang == true) ? $this->lang->line($msg_for) : $msg_for;
        return $this->response([
